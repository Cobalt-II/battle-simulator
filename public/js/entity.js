import {healercircle, base, ents} from '/js/config.js';
        ...
        case 'norm':
            ...
            for (let coun in entities) {
                if (entities[coun].team !== target.team) {
                    let k = Math.hypot(entities[coun].x - target.x, entities[coun].y - target.y);
                    targets.push([
                        [entities[coun].x, entities[coun].y, entities[coun].size], k
                    ]);
                }
            }
            if (targets.length) {
...
function getDeathAction(type, address) {
    switch (type) {
        case 'infect':
            entities[address] = new ent(Number(!entities[address].team), 'infect', entities[address].x, entities[address].y, entities[address].size, entities[address].maxhealth, entities[address].damage, entities[address].speed, entities[address].id);
            break;
        default:
    ...
    switch (type) {
        case 'heal':
            for (let u in entities) {
                if (Math.hypot(entities[address].x - entities[u].x, entities[address].y - entities[u].y) < healercircle.radius && entities[u].health < entities[u].maxhealth && entities[address].team === entities[u].team) {
...
requestAnimationFrame(function physics() {
    if (battleStarted) {
        for (let count in entities) {
            ...
            for (let coun in entities) {
                if (count !== coun && collision(entities[count].x, entities[coun].x, entities[count].y, entities[coun].y, entities[count].size, entities[coun].size) !== 'none') {
                    let angle = Math.abs(Math.atan2(entities[count].y - entities[coun].y, entities[count].x - entities[coun].x));
...
document.addEventListener('mousedown', function(event) {
    ...
    if (!battleStarted) {
        pushEnt(event.pageX > window.innerWidth / 2 ? teamsettings[1] : teamsettings[0], ents[targettype], event.pageX, event.pageY, h.size, h.health, h.damage, h.speed);
    }
});